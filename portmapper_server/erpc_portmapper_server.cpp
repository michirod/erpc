...
erpc_status_t portmapper_service::handleInvocation(uint32_t methodId, uint32_t sequence, Codec * codec, MessageBufferFactory *messageFactory, Transport *transport)
{
    ...
    switch (methodId)
    {
        case kportmapper_register_service_id:
            return register_service_shim(codec, messageFactory, sequence);

        case kportmapper_register_entry_id:
            return register_entry_shim(codec, messageFactory, sequence);

        case kportmapper_unregister_service_id:
            return unregister_service_shim(codec, messageFactory, sequence);

        case kportmapper_unregister_entry_id:
            return unregister_entry_shim(codec, messageFactory, sequence);

        case kportmapper_lookup_id:
            return lookup_shim(codec, messageFactory, sequence);

        case kportmapper_lookup_proto_id:
            return lookup_proto_shim(codec, messageFactory, sequence);
...
erpc_status_t portmapper_service::register_service_shim(Codec * codec, MessageBufferFactory *messageFactory, uint32_t sequence)
{
    ...
    if (!err)
    {
        result = register_service(program_id, protocol, core_id, port);
    }
...
erpc_status_t portmapper_service::register_entry_shim(Codec * codec, MessageBufferFactory *messageFactory, uint32_t sequence)
{
    erpc_status_t err = kErpcStatus_Success;
...
erpc_status_t portmapper_service::unregister_service_shim(Codec * codec, MessageBufferFactory *messageFactory, uint32_t sequence)
{
    erpc_status_t err = kErpcStatus_Success;
    ...
    if (!err)
    {
        result = unregister_service(program_id, protocol, core_id, port);
    }
...
erpc_status_t portmapper_service::unregister_entry_shim(Codec * codec, MessageBufferFactory *messageFactory, uint32_t sequence)
{
    erpc_status_t err = kErpcStatus_Success;
...
erpc_status_t portmapper_service::lookup_shim(Codec * codec, MessageBufferFactory *messageFactory, uint32_t sequence)
{
    erpc_status_t err = kErpcStatus_Success;
...
erpc_status_t portmapper_service::lookup_proto_shim(Codec * codec, MessageBufferFactory *messageFactory, uint32_t sequence)
{
    erpc_status_t err = kErpcStatus_Success;
    ...
    if (!err)
    {
        result = lookup_proto(program_id, protocol);
    }
